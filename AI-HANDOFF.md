...
| 5 | CI 컨트롤 + cosign 서명 | **부분 검증**; GitHub OIDC provenance 확인, `main` 보호 규칙은 미구성/미검증 |
...
- **Phase 5 — 부분 검증 (2026-09-21)**: SHA-pinned GitHub Actions workflow의 [run 35589420371](https://github.com/s1ns3nz0/ln-ssdf/actions/runs/35589420371)이 Node·Helm 검증과 keyless source-provenance bundle 발행을 통과했다. 다운로드한 bundle은 GitHub OIDC issuer, 정확한 `ci.yml@refs/heads/main` identity, repository·run ID·commit `d09bdcf7307840e6b78cf17e56d86e1988340de7` binding까지 검증했다. `scripts/verify-ci-provenance.sh`가 이 절차를 재현한다. 그러나 `main` branch protection은 구성·검증하지 않았고, 이 증거는 이미지 서명/SBOM/VSA가 아니다. 따라서 완료로 주장하지 않는다.
