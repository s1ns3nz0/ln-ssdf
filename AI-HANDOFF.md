...
| **5** | CI 검증 + source provenance 서명 | **완료 (source scope)**; `Validate` 보호 체크와 keyless provenance 검증 완료 |
| 6 | Kyverno Audit + 격리 namespace 차단 테스트 (+ CI `kyverno apply`) | C2P 실태 확인 지점 |
| 7 | Scorecard + 전체 배포 이미지 VSA + 해시체인/Rekor 기록 | **보류**; 7개 모두 signed VSA는 발행됐으나 정책상 FAILED |
...
- **Phase 5 — 완료 (source scope, 2026-09-21)**: SHA-pinned `Validate` workflow가 보호된 `main`에서 검증을 통과한 뒤, `Attest Source Provenance`가 그 정확한 commit만 checkout해 keyless bundle을 발행한다. [run 35591393479](https://github.com/s1ns3nz0/ln-ssdf/actions/runs/35591393479)의 bundle은 GitHub OIDC issuer, 정확한 `attest-source-provenance.yml@refs/heads/main` identity, repository·run ID·commit `dad63e1584206d0d9d55315f9daf75e425cbc32d` binding까지 검증했다. `main`은 `Repository verification`을 필수 체크로 요구한다. `scripts/verify-ci-provenance.sh`가 이 절차를 재현한다. 이 증거는 이미지 서명/SBOM/VSA가 아니다.
...
- **Phase 7 — 명시적 보류**: \`ssdf-system\`의 실제 7개 runtime image는 keyless CycloneDX SBOM·VSA가 발행되고 Cosign identity·transparency-log 검증까지 완료됐지만, 현재 취약점 정책의 blocking finding 때문에 VSA가 모두 \`FAILED\`다. versioned inventory가 이미지별 개수를 기록한다. [readiness gate](docs/phase-7.md)는 인벤토리 drift를 실패로, passing evidence 부재를 exit 3으로 처리한다. Scorecard는 lnd·aperture 두 source repository를 매일 측정하며, 최초 [run 35591904489](https://github.com/s1ns3nz0/ln-ssdf/actions/runs/35591904489)은 각각 5.8/10과 4.7/10이었다. 호환되는 이미지 업그레이드 뒤 fresh passing VSA를 만들기 전에는 Enforce로 승격하지 않는다.
