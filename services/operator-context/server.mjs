...
const allowed = { critical_alerts: 'count(ALERTS{alertstate="firing",severity="critical"})', healthy_targets: 'sum(up == 1)', ti_failure_rate: 'sum(increase(ti_product_failures_total[15m])) / clamp_min(sum(increase(ti_product_authorized_accesses_total[15m])) + sum(increase(ti_product_failures_total[15m])), 1)' };
let alerts = [];
...
createServer(async (req, res) => { try {
  ...
  if (r.id === undefined) { res.writeHead(202, { 'cache-control': 'no-store' }); return res.end(); }
