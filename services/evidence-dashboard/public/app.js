...
function inspector() {
  ...
  if (state.tab === 'posture') body = '<div class="split-status"><div>Implementation' + badge(selected.declaredStatus) + '</div><div>Current posture' + badge(selected.currentPosture) + '</div><div>Evidence freshness' + badge(evidenceLabels[selected.telemetryStatus] || selected.telemetryStatus) + '</div></div><p class="muted">' + esc(selected.telemetryReason) + '</p>' + (selected.blockers[0] ? '<div class="reason"><b>' + esc(selected.blockers[0].type) + '</b><p>' + esc(selected.blockers[0].detail) + '</p></div>' : '') + recordsPanel;
  ...
  if (state.tab === 'evidence') body = '<p><b>Runtime evidence · Observed activity:</b> ' + (selected.otel ? esc(selected.otel.log.message) : esc(evidenceLabels[selected.telemetryStatus] || selected.telemetryStatus)) + '</p><p><b>Latest assessment:</b> ' + esc(selected.assessments.at(-1).result) + ' · ' + esc(selected.assessments.at(-1).summary) + '</p><details class="record-details"><summary>Assessment history</summary>' + selected.assessments.map((assessment) => '<p class="assessment-row">' + badge(assessment.result) + '<b>' + esc(assessment.assessor) + '</b> ' + esc(assessment.summary) + '</p>').join('') + '</details>' + recordsPanel;
