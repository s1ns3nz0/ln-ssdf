...
function inspector() {
  ...
  if (state.tab === 'evidence') body = '<p><b>Runtime evidence:</b> ' + (selected.otel ? esc(selected.otel.log.message) : esc(evidenceLabels[selected.telemetryStatus] || selected.telemetryStatus)) + '</p><p><b>Latest assessment:</b> ' + esc(selected.assessments.at(-1).result) + ' · ' + esc(selected.assessments.at(-1).summary) + '</p><details class="record-details"><summary>Assessment history</summary>' + selected.assessments.map((assessment) => '<p class="assessment-row">' + badge(assessment.result) + '<b>' + esc(assessment.assessor) + '</b> ' + esc(assessment.summary) + '</p>').join('') + '</details>' + recordsPanel;
